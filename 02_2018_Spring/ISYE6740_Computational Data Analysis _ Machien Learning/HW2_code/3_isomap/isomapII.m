         ...
         if n_fcn == 'k'
             ...
             for i=1:N
                 ...
                 if ((verbose == 1) & (rem(i,50) == 0)), 
                     disp([' Iteration: ' num2str(i) '     Estimated time to completion: ' num2str((N-i)*toc/60/50) ' minutes']); 
                     tic; 
                 end
             end
         elseif n_fcn == 'epsilon'
...
%%% my part done         
    ...
    if ((mode==1) & (use_dijk == 0))
        ...
        if exist('dijkstra_sp.m','file'),
            D = dijkstra_sp(sparse(D), landmarks);
 %%% my part start 
        else
 ...
 %%% my part end
    ...
    if (displ==1)
         ...
         %%%%% Plot two-dimensional configuration %%%%%   -- 2d embeddings
         twod = find(dims==2);  
         if ~isempty(twod)
             figure;
             hold on;
