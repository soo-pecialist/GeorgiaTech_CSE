...
% and k-medoids in the other files, mykmeans.m and mykmedoids.m. Have fun!
    ...
    rows = size(image, 1); %214
    cols = size(image, 2); %320
    pixels = zeros(rows*cols, 3); %68480 x 3
    %size(image)
    %size(rows)
    %size(cols)
    %size(pixels)
    ...
    fprintf('\n')
    disp('Centroid for Kmeans(centroid1):'); disp(centroid1);
    fprintf('\n')
    disp('Centroid for Kmoids(centroid2):'); disp(centroid2);
